...
| 预览版 | `vX.Y.Z-rc.N` | `develop` | true | false | 客户端 / web / 移动 / NAS，**不含浏览器扩展** |
...
# 4) 工作树干净、停在目标分支的目标提交（稳定=main，预览=develop）
...
# 先切到正确分支：稳定版 main，预览版 develop（CI 守卫会拒绝打错分支的 tag）。
git checkout main        # 预览版改为: git checkout develop
git tag -a "$V" -m "$V"
git push origin "$V"
# 观察流水线
gh run watch
