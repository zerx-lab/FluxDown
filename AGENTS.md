...
| `popup_window_service.dart` | 外部唤起独立小窗（主引擎侧）。原生窗口承载**第二 Flutter 引擎**（entrypoint 参数 `--quick-popup`，零插件、不初始化 Rust），懒创建常驻复用；载荷（主题 tokens/语言/队列/默认目录）JSON 注入，结果经原生中继回主引擎发信号。显示时序为 reveal 握手：show 只投递载荷（窗口保持隐藏），弹窗 Dart 首帧就绪后经 `reveal(height)` 一次到位「设高+显示」，原生 3s 兜底定时器保证极端情况下窗口仍弹出；小窗可见期间新请求经 `append` 合入现有表单（append 模式，原生返回 false 时主引擎自愈失步的可见标志）；另有 15min pending watchdog 兜底复位（relay 活动重置计时）。**清单预解析**：小窗提交单条 http(s) 链接时经原生通用透传 `relay`/`onRelay`（JSON 信封，见 popup_payload.dart）请求主引擎 ResolvePreview——无清单则主引擎代提交并关窗（零差异），命中清单则小窗原窗切换 ManifestSelectView（窗口经带 width 的 resize 放大到 780×620+标题栏，取消回表单并归位 520），确认经 groupSubmit 中继回主引擎发 CreateTaskGroup（referrer 由 pending 表回填）；预解析等待/90s 超时/取消收敛在 resolve_preview_client.dart，与新建下载对话框、快速下载回退对话框三入口共用。**托管缓冲（受理即拥有）**：清单视图期间到达的外部请求 + 弹窗可见期间的音视频轨对请求不进（Offstage 的）表单，由本服务完整缓冲（上限 20，满丢最旧），清单退出（manifestClosed 中继）实时冲刷回活表单、会话结束（确认建组/关闭/watchdog）冲刷为新会话——经 ExternalDownloadService 注入的 redispatch 重走完整分发链，任何路径不静默丢请求。原生宿主：windows/runner/popup_window_host.cpp、macos/Runner/PopupWindowHost.swift、linux/runner/popup_window_host.cc |
